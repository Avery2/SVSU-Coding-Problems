...
public class Encryption {
	public static void main(String[] args) throws IOException {
		...
		FileWriter enc = new FileWriter("encrypted.txt", true);//encryptor
		FileWriter dec = new FileWriter("decrypted.txt", true);//decryptor
		char[] msg={}, key, keyMsg, encr;
		int ln1=1, ln2=2;
		clearFile("encrypted.txt");
		clearFile("decrypted.txt");

		System.out.println("\nunencrypted.txt");
		Tools.PrintFile("unencrypted.txt");

		while (true) {
			try {
				msg = toCharArr(Tools.ReadLn("unencrypted.txt", ln1));
			} catch (java.util.NoSuchElementException e) {
				break;
			}
			key = toCharArr(Tools.ReadLn("unencrypted.txt", ln2));
			keyMsg = new char[msg.length];//create keyMmsg
		...
		ln2=2;
		System.out.println("\nencrypted.txt");
		Tools.PrintFile("encrypted.txt");
		while (true) {
			try {
				encr = toCharArr(Tools.ReadLn("encrypted.txt", ln1));
			} catch (java.util.NoSuchElementException e) {
				break;
			}
			key = toCharArr(Tools.ReadLn("encrypted.txt", ln2));
